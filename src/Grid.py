...
from Grid_aux import TriangularFace
...
class PointLocation:
    ...
    def get_coordinates(
            self,
            grid: Grid
    ) -> np.ndarray[float]:  # todo: may be better to already calculate on construction. Or declare and assign when called
        """
		Get the actual space coordinates of this point,
		given the grid vertices

		linear interpolation using barycentric coordinates
		"""

        v0 = grid.get_grid_vertex(self.face.indices[0])
        v1 = grid.get_grid_vertex(self.face.indices[1])
        v2 = grid.get_grid_vertex(self.face.indices[2])

        coords: np.ndarray[float] = (self.barycentric_cords[0] * v0) + (self.barycentric_cords[1] * v1) + (self.barycentric_cords[2] * v2)
...
class Segment:
	...
	def __init__(self, endpoints: Optional[np.ndarray[PointLocation]] = None,
				 timestamps: Optional[np.ndarray[float]] = None):
...
class CurveDescription:
    ...
    def __init__(self, path: PolygonalPath2D, grid: Grid):  # todo: if a curve is realtive to a grid, should it store a reference to it ? help on plot
        """
